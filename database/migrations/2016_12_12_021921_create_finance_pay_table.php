...
class CreateFinancePayTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('finance_pay', function (Blueprint $table) {
            $table->increments('id');
            $table->string('trader_account');//操盘账号
            $table->float('money')->default(0);//金额
            $table->integer('type')->default(0);//充值方式
